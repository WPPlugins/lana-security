...
class Lana_Security_Login_Logs_List_Table extends WP_List_Table{
	...
	public function __construct() {
		...
		parent::__construct( array(
			'singular' => 'security_login_logs',
			'plural'   => 'security_login_logs',
			'ajax'     => false
		) );
	...
	public function column_cb( $item ) {
		return sprintf( '<input type="checkbox" name="lana_security_login_log[]" value="%s" />', $item->id );
	}
	...
	public function column_default( $log, $column_name ) {
		switch ( $column_name ) {
			...
			case 'status' :
				...
				return sprintf( __( '(status code: %d)', 'lana-security' ), $log->status );
				break;
			case 'comment' :
				return $log->comment;
				break;
	...
	public function get_columns() {
		$columns = array(
			'cb'       => '',
			'username' => __( 'Username', 'lana-security' ),
			'status'   => __( 'Status', 'lana-security' ),
        ...
        <div class="tablenav <?php echo esc_attr( $which ); ?>">
			...
			<?php if ( 'top' == $which ) : ?>
                <div class="alignleft actions">
                    ...
                    <input type="hidden" name="page" value="lana-security-login-logs"/>
	...
	public function process_bulk_action() {
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if ( ! current_user_can( 'manage_lana_security_login_logs' ) ) {
				wp_die( __( 'You\'re not allowed to delete login logs!' ) );
			}

			if ( count( $_POST['lana_security_login_log'] ) > 0 ) {

				foreach ( $_POST['lana_security_login_log'] as $log_id ) {
					global $wpdb;
